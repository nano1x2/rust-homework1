use borsh::{BorshDeserialize, BorshSerialize}; // ✅ ONLY this line is needed
use solana_program::{
...
pub fn process_instruction(
    ...
    match instruction {
        CounterInstructions::Increment(amount) => {
            counter_account.counter = counter_account.counter.saturating_add(amount);
        }
        CounterInstructions::Decrement(amount) => {
            counter_account.counter = counter_account.counter.saturating_sub(amount);
        }
...
mod test {
    ...
    fn test_counter() {
        ...
        increment_instruction_data.extend_from_slice(&5u32.to_le_bytes());
        ...
        decrement_instruction_data.extend_from_slice(&3u32.to_le_bytes());
